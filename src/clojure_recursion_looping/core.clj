(ns clojure-recursion-looping.core
  (:gen-class))
...
(defn recursive-sum [so-far numbers]
  (let [first-number (first numbers)]
    (if first-number
      (recur (+ so-far first-number) (next numbers))
      so-far)))

;; Partitioning Grocery Bags
...
(defn- bag-sequences* [{:keys [current-bag bags] :as acc} stream]
  (cond
    ...
    (bag-sequences* (assoc acc
                    ...
                    (next stream))
    :else (bag-sequences* (update acc :current-bag conj (first stream))
                          (next stream))))
...
;; Large-Scale Grocery Partitioning with recur
...
(defn- robust-bag-sequences* [{:keys [current-bag bags] :as acc} stream]
  (cond
    ...
    (recur (assoc acc 
                  ...
                  :bags (conj bags current-bag))
           (next stream))
    :else (recur (update acc :current-bag conj (first stream))
                 (next stream))))
...
(defn grocery-verfication [input-items]
  ...
  (loop [remaining-stream stream
    ...
    (let [{:keys [current-bag bags]} acc]
      (cond 
        ...
        (recur (next remaining-stream)
               (assoc acc
                      :current-bag (conj current-bag (first remaining-stream))))))))
...
(defn naive-tree-sum [so-far x]
  (cond (not x) so-far
        ...
        (recur (recur so-far (first x)) (next x))))
